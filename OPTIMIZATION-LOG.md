...
| R4 | 2026-06-21 | 类型安全 | 1 找→1 确认（**强收敛信号**：correctness/safety 空间近枯竭）：profile.ts load() 把 domains 守卫从 `!p.domains` 收紧到「必须普通对象」——损坏/旧版 localStorage 存成 7/"x"/数组时回落 empty()，免 recordSignals 在原始值上 `domains[x] ||= {}` 崩（每次 pin/play 热路径、无上游 try/catch，会白屏）。与 heartbeat/skillForge 同款损坏存档护栏。build 净。 | (本轮) |
