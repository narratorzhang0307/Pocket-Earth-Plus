...
> 一句话：**FROST = 主 agent（CEO / 调度器），curators / JOT / AGENT-FORGE = 子 agent（专项员工）。**
...
|---|---|---|
| **上下文隔离** | 子 agent 有独立上下文，读的文件 / 中间推理**不回流**主对话，防「上下文污染」 | 每个 curator 是独立运行页（独立 state / 独立 RunTrace），FROST 控制台只拿「钉成功了吗」这种结论 |
| **任务委派** | 主 agent 打包任务上下文 → 启动子 agent；子 agent 只看到被传入的任务描述 | 路线 A：`onRun(target)` 把「跳到哪个 curator」这件事派下去；路线 B：`runFrost` 把这句话的意图派给意图处理器 |
| **结论汇总** | 子 agent 只回传**摘要**（500 行日志 → 1 行根因） | curator 跑完只回「suggest 描述子」+「确认才落地球」，不把中间 token 灌回主对话 |

> 嵌套深度：书里建议**子 agent 不超过 2 层**。FROST 是 `主 → 子`，到此为止，不再往下嵌。

主 agent vs 子 agent 的职责分工（书 §4.6）：

| | 主 agent（FROST） | 子 agent（curator / JOT / forge） |
|---|---|---|
...
2. `MusicAgentsTab.tsx`：① `import`；② `Running` 联合类型加成员；③ `if (running === 'xxx') return <XxxRunPage/>`；④ 若要进控制台列表，`GROUPS` 加一行；⑤ `RUN_BY_NAME['xxx-curator'] = 'xxx'`（或 hero 放 `HERO_BY_NAME`）。
...
|---|---|---|---|---|
| `music-curator` | 把音乐钉到歌手出身地 / 歌曲城市。Use when 点歌·歌单·音乐。 | RUN_BY_NAME→`music` | QUICK「来份歌单」· 列表 | MusicCuratorPage |
| `books-curator` | 把书钉到故事地 / 作者地 + 读完日期。Use when 读书·书单·某作者。 | →`books` | QUICK「翻翻我的书」· 列表 | BooksCuratorPage |
| `movies-curator` | 把电影钉到取景地 / 故事地。Use when 看片·影单。 | →`movies` | QUICK「聊聊电影」· 列表 | MoviesCuratorPage |
| `photos-curator` | 端侧整理相册，高价值照片钉地球。Use when 整理照片·相册。 | →`photos` | QUICK「整理相册」· 列表 | PhotosCuratorRunPage |
| `travel-curator` | 按喜好端侧规划行程，完成即钉星球。Use when 规划行程·去某地玩。 | →`travel` | 列表 | TravelRunPage |
| `jot`（**记一笔**） | 一句话/截图 → FROST 判书·影·行程·心情 → 钉到对应图层；记心情还能回望。Use when 不想先选 agent，随手记一切。**Not for**：明确要点歌/规划长行程时直接进专属 curator 更准。 | →`jot` | **QUICK「记一笔」**· 列表 | UniversalCaptureRunPage |
...
| 书里的工程主张 | 本 app | 说明 |
|---|---|---|
| CEO 委派 / 子 agent | ✅ 对齐 | FROST 派活给 curators |
| 上下文隔离 | ✅ 对齐 | 每 curator 独立运行页 + 独立 RunTrace |
| 单一职责 | ✅ 对齐 | 一个 curator 一件事 |
...
- **「这是不是黄佳书里的主-子 agent 关系？」** —— 是。FROST↔curators 就是 CEO 委派模型 / SubAgent 模式，连「只回传结论、上下文隔离、单一职责、最小权限、confirm 才落地」这些细则都对得上。
