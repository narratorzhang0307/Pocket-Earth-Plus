...
// 云脑优先用通义 Qwen（DashScope）；未配 DASHSCOPE_API_KEY 时回退 DeepSeek。
...
function frostLlm(env: Record<string, string>): Plugin {
  const QWEN_KEY = env.DASHSCOPE_API_KEY || env.QWEN_API_KEY || ''
  const DS_KEY = env.DEEPSEEK_API_KEY || ''
  const provider = QWEN_KEY ? 'dashscope' : 'deepseek'
  const KEY = QWEN_KEY || DS_KEY
  const MODEL = QWEN_KEY ? (env.QWEN_MODEL || 'qwen-plus') : undefined
  return {
